...
app.get("/auth/debug", async (req, res) => {
  ...
  try {
    // Check User Info
    const userRes = await axios.get(`https://graph.facebook.com/v18.0/me?fields=name,id&access_token=${token}`);
    const user = userRes.data;

    // Check Permissions
    const permRes = await axios.get(`https://graph.facebook.com/v18.0/me/permissions?access_token=${token}`);
    const grantedPerms = permRes.data.data.filter((p: any) => p.status === 'granted').map((p: any) => p.permission);

    // Check Accounts (with extra fields)
    const response = await axios.get(`https://graph.facebook.com/v18.0/me/accounts?fields=name,id,instagram_business_account,tasks,is_published,category&access_token=${token}`);
    ...
    if (manualId) {
        try {
            const manualRes = await axios.get(`https://graph.facebook.com/v18.0/${manualId}?fields=name,instagram_business_account{id,username}&access_token=${token}`);
    ...
    res.status(500).send(`<h3>❌ API Error</h3><pre>${JSON.stringify(e.response?.data || e.message, null, 2)}</pre>`);
  }
});
...
app.get("/auth/instagram/callback", async (req, res) => {
  ...
  try {
    ...
    res.status(500).send(`Error exchanging token: ${JSON.stringify(err.response?.data || err.message)}`);
  }
});
...
io.on("connection", (socket) => {
  ...
  socket.on("start-analysis", async (data) => {
    ...
    if (activeScans >= MAX_CONCURRENT) {
        socket.emit("log", `⚠️ Server is busy (${activeScans}/${MAX_CONCURRENT} active scans). Please try again in a minute.`);
    ...
    const compsArray = competitors.split(/[\s,]+/).map((c: string) => c.trim()).filter((c: string) => c.length > 0);
