...
async function getCommentersAPI(postUrl: string, accessToken: string, onLog: (m: string) => void): Promise<Set<string> | null> {
    try {
        onLog("📡 Fetching comments via Graph API...");
        
        // 1. Get IG Business Account ID
        const accountsRes = await axios.get(`https://graph.facebook.com/v18.0/me/accounts?fields=instagram_business_account,name&access_token=${accessToken}`);
        const pages = accountsRes.data.data || [];
        let page = pages.find((p: any) => p.instagram_business_account);
        ...
        if (!igId) {
            ...
            if (manualId) {
                ...
                try {
                    const manualRes = await axios.get(`https://graph.facebook.com/v18.0/${manualId}?fields=instagram_business_account,name&access_token=${accessToken}`);
        ...
        const mediaRes = await axios.get(`https://graph.facebook.com/v18.0/${igId}/media?fields=shortcode&limit=50&access_token=${accessToken}`);
...
// ... existing code ...
  ...
  try {
    browser = await puppeteer.launch({
      ...
      args: ["--start-maximized", "--disable-notifications", "--no-sandbox", "--disable-setuid-sandbox"],
    });
  } catch (e: any) {
...
export async function startAnalysis(
  ...
  try { // Wrap everything in try/finally to ensure cleanup
  ...
  if (commenters) {
    ...
    try {
        ...
        commenters = await scrapeCommenters(
          page,
          config.postUrl,
          config.competitors,
          onLog
  ...
  try {
    ...
    const competitorSet = new Set(
      config.competitors.map((c) => c.toLowerCase())
    );
...
async function scrapeCommenters(
  ...
  const usernames = await page.evaluate((competitorsList) => {
    ...
    const competitors = new Set(
      competitorsList.map((c: string) => c.toLowerCase())
    );
    const commentSpans = Array.from(
      document.querySelectorAll('div.html-div span[dir="auto"]')
    );
...
async function analyzeUser(
  ...
  try {
    ...
    const userId = await page.evaluate(() => {
      ...
      const match =
        html.match(/"profile_id":"(\d+)"/) || html.match(/"id":"(\d+)"/);
            ...
            async (targetId, comp, csrf, appId) => {
              try {
                ...
                if (csrf) headers["X-CSRFToken"] = csrf;
                            ...
                            return (
                              data.users?.some(
                                (u: any) => u.username.toLowerCase() === comp.toLowerCase()
                              ) || false
                            );              } catch (e) {
                return false;
              }
